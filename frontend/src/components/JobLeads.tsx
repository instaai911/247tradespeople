...
import { ArrowLeft, MapPin, Calendar, DollarSign, Users, Heart, CreditCard, CheckCircle, Clock, AlertTriangle, Filter, UserCheck, Star, X, MessageCircle } from 'lucide-react';
import { useApp } from '../context/AppContext';
import { Interest, Review, Conversation } from '../types';
...
const JobLeads = () => {
  ...
  const [selectedJobToDismiss, setSelectedJobToDismiss] = useState<string | null>(null);
  ...
  const filteredJobLeads = state.jobLeads.filter(lead => {
    if (isHomeowner) {
  ...
  const availableCategories = ['all', ...Array.from(new Set(state.jobLeads.map(lead => lead.category)))];
  ...
  const handlePurchaseLead = (leadId: string) => {
    ...
    dispatch({ type: 'PURCHASE_LEAD', payload: { leadId, userId: state.currentUser.id } });
  ...
  const submitInterest = () => {
    ...
    dispatch({ type: 'EXPRESS_INTEREST', payload: { leadId: selectedLead, interest } });
  ...
  const handleAcceptInterest = (leadId: string, interestId: string) => {
    ...
    if (interest?.price === 0) {
      alert('Interest accepted! No charge for VIP member. The tradesperson now has access to your contact details.');
    } else {
      alert(`Interest accepted! The tradesperson has been charged £${interest?.price.toFixed(2) || '5.99'} and now has access to your contact details.`);
    }
  };
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {viewMode === 'map' ? (
          ...
          {filteredJobLeads.map((lead) => {
            ...
            const hasPurchased = state.currentUser && lead.purchasedBy.includes(state.currentUser.id);
            const hasExpressedInterest = state.currentUser && lead.interests.some(
              interest => interest.tradespersonId === state.currentUser!.id
            );
            const isOwner = state.currentUser && lead.postedBy === state.currentUser.id;
            const hiredTradesperson = lead.hiredTradesperson ? state.users.find(u => u.id === lead.hiredTradesperson) : null;

            return (
              <div key={lead.id} className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 relative">
                ...
                <div className="flex justify-between items-start mb-4">
                  ...
                  <div className="flex items-center space-x-2">
                    ...
                    <div className={`px-3 py-1 rounded-full text-sm font-medium flex items-center ${getUrgencyColor(lead.urgency)}`}>
                ...
                <p className="text-gray-600 mb-4 line-clamp-3">{lead.description}</p>
                ...
                <div className="flex items-center justify-between mb-4">
                  <span className="text-lg font-bold text-blue-600">£{lead.price}</span>
                ...
                {hiredTradesperson && (
                  <div className="bg-green-50 border border-green-200 rounded-lg p-4 mb-4">
                    <div className="flex items-center justify-between">
                      <div>
                        <h4 className="font-semibold text-green-800 mb-1">Hired Tradesperson</h4>
                        <p className="text-green-700">{hiredTradesperson.name}</p>
                        <p className="text-sm text-green-600">{hiredTradesperson.trades?.join(', ')}</p>
                      </div>
                      {isOwner && (
                        <button
                          onClick={() => handleLeaveReview(lead.id, hiredTradesperson.id)}
                ...
                {isOwner && lead.purchasedBy.length > 0 && (
                  <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-4">
                    <h4 className="font-semibold text-blue-800 mb-2">Tradespeople who purchased this lead</h4>
                      ...
                      return (
                        <div key={tradespersonId} className="flex items-center justify-between bg-white rounded-lg p-3 mb-2 last:mb-0">
                          ...
                          {lead.isActive && !lead.hiredTradesperson && (
                            <button
                              onClick={() => handleHireTradesperson(lead.id, tradespersonId)}
                ...
                {lead.interests.length > 0 && (
                  <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-4">
                    <h4 className="font-semibold text-blue-800 mb-2">Expressed Interests</h4>
                    {lead.interests.map((interest) => (
                      <div key={interest.id} className="text-sm text-blue-700 mb-2 last:mb-0">
                        <div className="flex justify-between items-start">
                          <div>
                            <p><strong>{interest.tradespersonName}</strong></p>
                            <p className="text-blue-600">{interest.message}</p>
                            <p className="text-xs text-blue-500">{interest.date}</p>
                          </div>
                          <div className="flex space-x-2">
                            {isOwner && interest.status === 'pending' && lead.isActive && (
                              <button
                                onClick={() => handleAcceptInterest(lead.id, interest.id)}
                                className="bg-green-600 text-white px-3 py-1 rounded text-xs hover:bg-green-700"
                              >
                                Accept
                              </button>
                            )}
                            {isOwner && interest.status === 'accepted' && lead.isActive && !lead.hiredTradesperson && (
                              <button
                                onClick={() => handleHireTradesperson(lead.id, interest.tradespersonId)}
                            ...
                            {!isHomeowner && interest.tradespersonId === state.currentUser?.id && interest.status === 'accepted' && (
                              <button
                                onClick={() => {
                ...
                <div className="flex space-x-3">
                  ...
                  {!isHomeowner && !hasPurchased && canPurchase && lead.isActive && (
                    <>
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white rounded-xl p-6 max-w-md w-full mx-4">
              <div className="flex items-center mb-4">
                ...
                <h3 className="text-lg font-semibold text-gray-900">Dismiss Job</h3>
              </div>
              <p className="text-gray-600 mb-6">
                Are you sure you want to dismiss this job? It will be permanently hidden from your job feed and you won't be able to see it again.
              </p>
